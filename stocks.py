...
parser.add_argument(
    "-s",
    dest="source",
    help="Source system of the statement files. Default: [revolut]",
    default="revolut",
    required=False,
)
...
def main():
    ...
    parser = None
    try:
        parser = globals()[parsed_args.source].Parser(parsed_args.input_dir)
        statements = parser.parse()
    except KeyError:
        logger.error(f"Unsupported source system: [{parsed_args.source}].")
        raise SystemExit(1)
